...
import sk.uniba.fmph.Burnie.Controller.ControllerException;
...
public class ControllerHandler extends Thread {
    ...
    public void changeControllerParameters(int temperature, short airFlow, long time) throws IOException { //TODO -> it might overflow
        controller.setTime(time);
        controller.setAirFlow(airFlow);
        controller.setCurrentTemperature(temperature);

        ByteBuffer tempBuffer = ByteBuffer.allocate(4), airFlowBuffer = ByteBuffer.allocate(2), timeBuffer = ByteBuffer.allocate(8);
            ...
            try {
                ...
                if (flags == 0b0000010) {
                    ...
                    if ((flags&0b00010000) > 0) {
                        throw new ControllerException(""); //TODO -> what is in progress
                    }
