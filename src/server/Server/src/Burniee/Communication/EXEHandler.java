    ...
    public void run() {
        ...
        while (socket.isActive()) {
            try {
                ...
                GeneralLogger.writeMessage("[EXE] received a message from EXE");
                if (MessageBuilder.EXE.FileTransfer.equals(msg)) {
                    System.out.println("[EXE] starting a file transfer");
                    GeneralLogger.writeMessage("[EXE] starting a file transfer");
                    ...
                    GeneralLogger.writeMessage("[EXE] new Project with id = " + projectID + " is about to start");
                    ...
                    GeneralLogger.writeMessage("[EXE] End of phase message for project with id = " + id + " arrived!");
                    for (Project p : Server.getInstance().getActiveProjects()) {
                        if (p.getID().equals(id)) {
                            ...
                            GeneralLogger.writeMessage("[EXE] Found the right project and confirmed end of phase");
                            p.confirmEndOfPhase();
                            socket.stopSocket();
                            return;
                        }
